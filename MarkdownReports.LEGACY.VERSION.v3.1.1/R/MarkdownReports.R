...
wplot <- function (df_2columns, col = 1, pch = 18, ..., panel_first=grid(NULL), plotname = substitute(df_2columns),
                   errorbar = F, upper = 0, lower = upper, left = 0, right = left, width = 0.1, arrow_lwd = 1, col_errorbar = 1, ylim=F, xlim=F,
  ...
  if (errorbar) {
    ylim_ = range(c( (y + upper + abs(0.1 * y)), (y - lower - abs(0.1 * y))), na.rm = T)
    xlim_ = range(c( (x + right + abs(0.1 * x)), (1.1 * x - left - abs(0.1 * x))), na.rm = T)
  }
  else {
    ylim_ = range(y, na.rm = T)
    xlim_ = range(x, na.rm = T)
  }
  if (equal.axes) xlim_ = ylim_ = range(c(xlim_,ylim_))
  if (is.numeric(ylim) & length(ylim)==2) { ylim_ = ylim } #overwrite if
  if (is.numeric(xlim) & length(xlim)==2) { xlim_ = xlim }

  plot(df_2columns, ..., main = plotname, col = col, pch = pch, ylim = ylim_, xlim = xlim_, panel.first= panel_first)
  if (errorbar) {
